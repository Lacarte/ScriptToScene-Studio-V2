<!-- GENERATED FILE — DO NOT EDIT BY HAND.
     ...
     (or: python -m studio.shared.providers_common.docs)
     Source of truth: studio.shared.providers_common domains + hub -->
...
- `studio/<domain-package>/providers/<provider_id>/manifest.py` — `ProviderManifest` with `contract_version=2`.
...
The command refuses unknown domains, invalid ids, existing packages, and colliding test files. Failure is atomic — no partial provider folder is left behind. Use `python -m studio.shared.providers_common.scaffold --help` for options.
...
Every package exports `create()` — a zero-arg factory returning the provider instance. Domain bases live in `studio/<package>/providers/base.py`. Prefer Contract v2 `invoke()` (or the media-job service for storyboard/animator); the concrete domain seams remain for legacy adapters.

| Domain | Shape | Concrete seam |
|---|---|---|
| `script` | sync document | `generate(configuration, project_id=…)` → `invoke(request, invocation)` |
| `scene_blueprint` | sync document | `generate(segments, configuration, project_id=…)` → `invoke(…)` |
| `tts` | sync artifact | `synthesize(text, settings, …)` → `invoke(…)` |
| `storyboard` | async multi-asset | `submit` / `poll` (media-job service) |
| `animator` | async multi-asset | `submit` / `poll` (media-job service) |

### Sync document example (script / scene_blueprint)
...
### Async multi-asset example (storyboard / animator)
...
One envelope serves all five domains: `ProviderResult` (`studio.shared.providers_common.results`). Only `payload` varies by domain. Status is `succeeded`, `partial`, or `failed`.
...
Run the generated contract tests first, then add domain-specific cases. Reusable suites and offline fakes live in `studio.shared.providers_common.contract_tests`.
...
2. Run generated + domain tests; regenerate docs (`python -m studio.workflows.docs`).
...
- **Providers folder:** `studio/story/providers`
...
#### Request model (`studio.story.providers.contract:ScriptRequest`)
...
#### Result payload (`studio.story.providers.contract:ScriptResultPayload`)
...
### Scene Blueprint (`scene_blueprint`)

- **Default provider:** `n8n`
- **Package:** `studio.build_scene_blueprints.providers`
- **Providers folder:** `studio/build_scene_blueprints/providers`
- **Execution shape:** sync document
...
#### Request model (`studio.build_scene_blueprints.providers.contract:SceneBlueprintRequest`)
...
#### Result payload (`studio.build_scene_blueprints.providers.contract:SceneBlueprintResultPayload`)
...
- **Providers folder:** `studio/tts/providers`
...
#### Request model (`studio.tts.providers.contract:TTSRequest`)
...
#### Result payload (`studio.tts.providers.contract:TTSResultPayload`)
...
- **Providers folder:** `studio/storyboard/providers`
...
#### Request model (`studio.storyboard.providers.contract:StoryboardRequest`)
...
#### Result payload (`studio.storyboard.providers.contract:StoryboardResultPayload`)
...
- **Providers folder:** `studio/animator/providers`
- **Execution shape:** async multi-asset
- **Concrete seam:** `submit` / `poll` (media-job service)
...
#### Request model (`studio.animator.providers.contract:AnimatorRequest`)
...
#### Result payload (`studio.animator.providers.contract:AnimatorResultPayload`)
...
| `scene_blueprint` | `n8n` |
| `tts` | `inworld`, `kokoro` |
| `storyboard` | `gemini_ws`, `wavespeed_direct`, `wavespeed_webhook` |
...
| Docs check fails | Manifest/domain change without regen | `python -m studio.workflows.docs` then commit `docs/providers.md` and `docs/provider-author-guide.md` |
