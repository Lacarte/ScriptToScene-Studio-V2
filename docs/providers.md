...
This document is generated from the domain catalog (`studio.shared.providers_common.domains`) and the process-wide hub (`studio.shared.providers_common.hub`). It is the same discovery surface served by `GET /api/providers`. Music and Captions are deliberately **not** provider domains — they remain local services without a provider dimension.
...
|---|---|---|---|---|
| `script` | Script / Story | `gemini` | `studio.story.providers` | sync document |
| `scene_blueprint` | Scene Blueprint | `n8n` | `studio.build_scene_blueprints.providers` | sync document |
| `tts` | Text to Speech | `kokoro` | `studio.tts.providers` | sync artifact |
| `storyboard` | Storyboard | `gemini_ws` | `studio.storyboard.providers` | async multi-asset |
| `animator` | Animator | `grok_automa` | `studio.animator.providers` | async multi-asset |
