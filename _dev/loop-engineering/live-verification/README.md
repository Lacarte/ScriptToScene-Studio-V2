...
## Provider status (verified 2026-08-05)

| Provider | Status | Notes |
|---|---|---|
| Kokoro TTS (local ONNX) | ✅ live-verified | `voice.wav` in the project TTS dir; service-level cache by text fingerprint |
| stable-whisper tiny.en | ✅ live-verified | Word spans use `begin`/`end` keys; zero-duration words post-fixed |
| Segmenter (local) | ✅ live-verified | 5 scenes from the 48-word script |
| Scene blueprint (n8n + OpenRouter) | ✅ live-verified | Hosted Railway webhook is dead (workflow inactive + API key revoked + **OpenRouter balance negative** → paid models return HTTP 402). Local n8n + free model works |
| WaveSpeed storyboard | ❌ blocked | Key rejected on every model (HTTP 401 "Invalid API key"). Node-failure path (`STORYBOARD_FAILED`) was exercised live. Replace the key, then set `STS_LIVE_STORYBOARD=1` |
| Kie AI animator | ✅ live-verified | 5/5 images generated + downloaded (nano-banana; per-image latency 10 s–2.5 min) |
| grok_automa animator | ⏭ not automatable | Requires a human-driven Automa browser session |
| FFmpeg export | ✅ live-verified | Playable 1080×1920 mp4 with video+audio streams (ffprobe-checked) |
