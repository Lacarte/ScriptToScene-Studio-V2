...
from studio.shared.providers_common.scaffold import (
    ScaffoldError,
    build_parser,
    scaffold_provider,
)
...
def test_scaffolder_refuses_collision_without_partial_files(tmp_path):
    ...
    provider_dir = (
        tmp_path / "studio" / "tts" / "providers" / "once_only"
    )
...
def test_scaffolder_atomic_failure_cleans_partial_writes(tmp_path, monkeypatch):
    ...
    from studio.shared.providers_common import scaffold as scaffold_mod
    ...
    provider_dir = tmp_path / "studio" / "story" / "providers" / "partial_fail"
