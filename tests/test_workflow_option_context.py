...
from studio.shared.providers_common import domains as domains_module
from studio.shared.providers_common import settings_manager
from studio.shared.providers_common.hub import ProviderHub
from studio.workflows import options as workflow_options
...
from studio.workflows.registry import ASYNC_OPTION_SOURCES, OptionSourceSpec
...
class ContextValidationTests(OptionContextTestCase):
    ...
    def test_a_domain_outside_the_source_scope_is_rejected(self):
        # `tts_voices?domain=storyboard` is a nonsense pairing the client could
        # otherwise cache and save.
        with self.assertRaises(OptionContextError):
            build_context('tts_voices', {'domain': 'scene_blueprint'})
...
class PerProviderResolutionTests(OptionContextTestCase):
    ...
    def test_every_domain_has_a_providers_source(self):
        ...
        for domain in ('script', 'scene_blueprint', 'tts', 'storyboard', 'animator'):
...
class ParityTests(unittest.TestCase):
    ...
    def test_every_spec_declares_a_known_cache_policy(self):
        from studio.workflows.registry import CACHE_POLICIES
