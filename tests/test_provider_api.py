...
from studio.shared.providers_common import settings_schema as ss
...
from studio.shared.providers_common.runtime import RuntimeBinding
...
class CatalogTests(ProviderApiTestCase):
    ...
    def test_the_catalog_enumerates_the_deprecated_provider_identities(self):
        ...
        self.assertEqual(aliases['gemini'], ('storyboard', 'gemini_ws'))
        self.assertEqual(aliases['webhook'], ('storyboard', 'wavespeed_webhook'))
        self.assertEqual(aliases['direct'], ('storyboard', 'wavespeed_direct'))
        self.assertEqual(aliases['grok'], ('animator', 'grok_automa'))
        self.assertEqual(aliases['kie-ai'], ('animator', 'kie_ai'))

        # And a deprecated identity resolves through the API to its canonical id.
        body = self.client.get('/api/providers/storyboard/gemini').get_json()
...
class StartupRuntimeTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.calls = []
        # `patch('studio.shared.providers_common.hub....')` would resolve `hub` to
...
class BlueprintRegistrationTests(unittest.TestCase):
    def test_the_editor_blueprint_no_longer_owns_a_provider_route(self):
    ...
    def test_both_blueprints_coexist_on_one_app(self):
        ...
        # The editor keeps the app-config store; the provider blueprint keeps
