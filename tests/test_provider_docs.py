...
from studio.shared.providers_common.errors import PROVIDER_CODES
from studio.shared.providers_common.hub import hub
from studio.shared.providers_common.settings_schema import WIDGET_TYPES
from studio.workflows.docs import main as workflow_docs_main
...
class ProviderReferenceGenerationTests(unittest.TestCase):
    ...
    def test_committed_reference_matches_generated_output(self):
        self.assertTrue(
            ...
            "docs/providers.md is missing — run python -m studio.workflows.docs",
        )
        self.assertEqual(
            DEFAULT_REFERENCE_OUTPUT.read_text(encoding="utf-8"),
            self.content,
            "docs/providers.md is stale — regenerate with: python -m studio.workflows.docs",
        )
