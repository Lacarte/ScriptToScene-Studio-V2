...
from studio.shared.providers_common.hub import ProviderHub, bind_domain, hub
from studio.shared.providers_common.registry import ProviderManifest, ProviderRegistry
...
class DomainCatalogTests(unittest.TestCase):
    ...
    def test_every_spec_is_complete(self):
        for domain_id, spec in DOMAINS.items():
            ...
            self.assertTrue(spec.package.startswith('studio.'))
...
class HubSurfaceTests(unittest.TestCase):
    ...
    def test_catalog_serializes_every_domain(self):
        ...
        self.assertEqual([p['id'] for p in payload['script']['providers']], ['builtin'])
    ...
    def test_shipped_domains_resolve_their_providers(self):
        ...
        self.assertIsNotNone(hub.get('storyboard', 'gemini_ws'))
        self.assertIsNotNone(hub.get('animator', 'kie_ai'))
...
class CompatibilityFacadeTests(unittest.TestCase):
    ...
    def test_tts_package_exposes_the_hub_registry(self):
        from studio.tts import providers as tts_providers
    ...
    def test_storyboard_and_animator_packages_expose_the_hub_registry(self):
        from studio.animator import providers as animator_providers
        from studio.storyboard import providers as storyboard_providers

        self.assertIs(storyboard_providers.registry, hub.registry('storyboard'))
        self.assertIs(animator_providers.registry, hub.registry('animator'))
...
class BrokenProviderIsolationTests(unittest.TestCase):
    ...
    def test_broken_providers_are_excluded_individually(self):
        ...
        _write_provider(self.base, 'fff_wrong_domain', domain='animator')
    ...
    def test_wrong_domain_registration_is_refused(self):
        manifest = ProviderManifest(
            id='alpha', label='Alpha', domain='animator', kind='local', version='1.0.0',
            capabilities={},
        )
        self.registry.register('alpha', module=None, manifest=manifest)
        self.assertEqual(self.registry.list_ids(), [])
