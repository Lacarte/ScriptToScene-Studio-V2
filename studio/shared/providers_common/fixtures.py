...
(`studio/workflows/fixtures/`, frozen by step 2.5). Port fixtures describe what
...
`studio/workflows/fixtures/media/`, so the repo keeps one copy of every byte.
"""
...
def _validate_expected_result(payload: Any) -> list[str]:
    ...
    for ref in payload.get("artifact_refs") or []:
        if isinstance(ref, str) and ref.startswith(MEDIA_ROOT):
            media = os.path.join(
                ROOT_DIR, "studio", "workflows", "fixtures", ref
            )
