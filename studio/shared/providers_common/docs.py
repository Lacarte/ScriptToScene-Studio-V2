...
Usage:
    python -m studio.shared.providers_common.docs            # rewrite both files
    python -m studio.shared.providers_common.docs --check    # exit 1 if stale

Also integrated into ``python -m studio.workflows.docs`` so one drift gate covers
...
from studio.shared.providers_common.errors import PROVIDER_CODES, RETRYABLE
from studio.shared.providers_common.hub import hub
from studio.shared.providers_common.scaffold import DOMAIN_SHAPE
from studio.shared.providers_common.settings_schema import (
    REDACTION_SENTINEL,
    SENSITIVE_KEYS_RE,
    WIDGET_TYPES,
)
from studio.shared.providers_common.validation import KINDS
...
_SEAM_BY_DOMAIN = {
    ...
    "scene_blueprint": "`generate(segments, configuration, project_id=…)` → `invoke(…)`",
    "tts": "`synthesize(text, settings, …)` → `invoke(…)`",
    "storyboard": "`submit` / `poll` (media-job service)",
    "animator": "`submit` / `poll` (media-job service)",
}
...
def generate_provider_reference() -> str:
    ...
    lines: list[str] = [
        ...
        "(`studio.shared.providers_common.domains`) and the process-wide hub "
        "(`studio.shared.providers_common.hub`). It is the same discovery surface "
...
def generate_provider_author_guide() -> str:
    ...
    lines: list[str] = [
        ...
        "editing `studio/workflows/`, a shared Vue provider component, or a "
        ...
        "This creates:",
        "",
        "- `studio/<domain-package>/providers/<provider_id>/manifest.py` — "
        ...
        "left behind. Use "
        "`python -m studio.shared.providers_common.scaffold --help` for options.",
        "",
        "## 2. Manifest",
        "",
        ...
        "`studio/<package>/providers/base.py`. Prefer Contract v2 `invoke()` "
        "(or the media-job service for storyboard/animator); the concrete domain "
    ...
    lines.extend([
        ...
        "(`studio.shared.providers_common.results`). Only `payload` varies by domain. "
    ...
    if args.check:
        ...
        if stale:
            print(
                ...
                "Run: python -m studio.shared.providers_common.docs"
            )
            return 1
