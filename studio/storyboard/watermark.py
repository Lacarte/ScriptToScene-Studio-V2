...
Removes the Gemini AI sparkle watermark by reversing the alpha compositing
using embedded alpha maps extracted from the watermark pattern.

Algorithm based on https://github.com/allenk/GeminiWatermarkTool (MIT License).
No external binary needed -- pure Python with Pillow + NumPy.
"""
...
# Embedded alpha maps (48x48 and 96x96) for the Gemini sparkle watermark.
# Extracted from GeminiWatermarkTool embedded assets.
...
def _decode_alpha_map(b64_data):
    """Decode a base64-encoded PNG into a float32 alpha map (0.0-1.0)."""
...
def _get_alpha_48():
    ...
    if _alpha_48 is None:
        _alpha_48 = _decode_alpha_map(_BG_48_B64)
    return _alpha_48
...
def _get_alpha_96():
    ...
    if _alpha_96 is None:
        _alpha_96 = _decode_alpha_map(_BG_96_B64)
    return _alpha_96


def _get_watermark_config(w, h):
    """Select watermark size and position based on image dimensions."""
    if w > 1024 and h > 1024:
        return {
            "margin_right": 64,
            "margin_bottom": 64,
            "logo_size": 96,
            "alpha_fn": _get_alpha_96,
        }
    return {
        "margin_right": 32,
        "margin_bottom": 32,
        "logo_size": 48,
        "alpha_fn": _get_alpha_48,
    }
...
def remove_watermark(filepath: str) -> bool:
    ...
    Uses reverse alpha blending -- deterministic, fast, no external binary.
    ...
    try:
        ...
        cfg = _get_watermark_config(w, h)
        alpha_map = cfg["alpha_fn"]()
        logo_size = cfg["logo_size"]

        # Watermark position: bottom-right with margin
        pos_x = w - cfg["margin_right"] - logo_size
        pos_y = h - cfg["margin_bottom"] - logo_size

        # Clip to image bounds
        x1, y1 = max(0, pos_x), max(0, pos_y)
        x2, y2 = min(w, pos_x + logo_size), min(h, pos_y + logo_size)

        # Corresponding region in the alpha map
        ax1, ay1 = x1 - pos_x, y1 - pos_y
        ax2, ay2 = ax1 + (x2 - x1), ay1 + (y2 - y1)

        roi = img_arr[y1:y2, x1:x2]
        alpha = alpha_map[ay1:ay2, ax1:ax2]

        # Reverse alpha blending: original = (composited - alpha * white) / (1 - alpha)
        alpha_3ch = alpha[:, :, np.newaxis]
        one_minus_alpha = np.clip(1.0 - alpha_3ch, 0.01, 1.0)
        original = (roi - alpha_3ch * 255.0) / one_minus_alpha
        original = np.clip(original, 0, 255).astype(np.uint8)

        img_arr[y1:y2, x1:x2] = original

        result = Image.fromarray(img_arr.astype(np.uint8))
        fmt = filepath.rsplit(".", 1)[-1].lower()
        save_fmt = {"jpg": "JPEG", "jpeg": "JPEG", "png": "PNG"}.get(fmt, "JPEG")
        result.save(filepath, format=save_fmt, quality=95)
